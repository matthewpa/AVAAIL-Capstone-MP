...
MODEL_DIR = "models"
MODEL_VERSION = 0.5
MODEL_VERSION_NOTE = "shift to simple neural network"
...
def model_train(data_dir,test=False):
    ...
    for country,df in ts_data.items():
        ...
        error_margin, params = _model_train(df,country,test=test)
        result_view.append([country, error_margin, params])

    return result_view

def model_load(prefix='sl',data_dir=None,training=True,version=MODEL_VERSION):
    """
    ...
    all_models = {}
    for model in models:
        req_version = re.sub("\.","_",str(version))
        load_version = re.split("\.", re.split("-",model)[2])[0]
        if(not req_version == load_version):
            continue
        if model.endswith('.h5'):
            all_models[re.split("-",model)[1]] = tf.keras.models.load_model(os.path.join(".","models",model))
        else:
...
def model_predict(country,year,month,day,all_models=None,test=False, all_data=None):
    """
