...
from model import model_train, model_train_dataframe, model_load, model_predict
...
def predict():
    ...
    if not ('country' in query):
        print("ERROR API (predict): missing parameter")
        return jsonify([])
    ...
    ## load model
    model = model_load(prefix='sl',data_dir=None,training=False, version=version)

    if not model:
        print("ERROR: model is not available")
        return jsonify([])

    _result = model_predict(query.country, query.year, query.month, query.day, test=test)
    result = {}
...
def train():
    ...
    if not ('data' in request.json):
        print("ERROR: API (train): did not receive training dataset")
        return jsonify(False)

    df = request.json["data"]
    print("... training model")
    model = model_train_dataframe(tag, df,test=test)
